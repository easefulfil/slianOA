...
//	        fitColumns:true,
	        ...
	        columns: [[
	                   ...
	                   { field:'cname', title: '联系人姓名', width: 100, sortable: true,halign:'center',},
	                   { field:'sex', title: '性别', width: 50, sortable: true,halign:'center',},
	                   { field:'mobile', title: '手机', width: 120, sortable: true,halign:'center',},
	                   { field:'hometel', title: '座机', width: 120, sortable: true,halign:'center',},
	                   { field:'duty', title: '职位', width: 100, sortable: true,halign:'center',},

	                   { field : 'operate',title : '操作',width : 120,sortable : true,
	   						formatter:
//	   							function(){return '<a class="l-btn l-btn-plain easyui-tooltips" href="javascript:void(0)" title="点击这里查看和修改其他费用" onclick="otherfee(\'' + row.id+ '\')">其他费用</a>'},
		...
		toolbar : [ {
			...
			id:"",//todo definate RESOURCE_4050500
			text : '新建联系人',
			iconCls : 'icon-head1',
			handler : function() {

				showDiv_customerEmployee('insert');
...
//				alert("更改负责");
				...
				show_dialog_customer_track(rows[0].customerId);
...
function setCustomerType(customerIds,type){
	...
	$.ajax({
		type : 'POST',
		url : getContextPath() +'/customer/setvip.do',///pcbas-analysis
		data : {
...
//			datagrid.datagrid('load');
		...
		function tableexport() {
			var form=$("#searchForm");
			form.attr("action","/salesys-pc-web/customer/exportExcel.do");
			form.submit();
		}
		function add() {
			if (editRow != undefined)  {
