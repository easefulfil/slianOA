...
function insertClick(){ 
	...
	var asset  = JSON.stringify(sy.serializeObject($('#oaAssetForm')));
		//var resourcesId=document.getElementById("id").value;
		//检查用户输入的商户编号是否符合要求
		$.ajax({
			  url : getContextPath()+'/oaAsset/insert.do',//resources/insert.do', 
		      type : 'POST', 
		      data:{"asset":asset},
		      dataType : 'json',
		      async : false,
...
//	var theId=document.getElementById("resourceid").value;
    $.ajax({ 
        url : getContextPath()+'/oaAsset/update.do', 
        type : 'POST', 
        data:{"asset":asset},// ,"basicDataId":$('#basicDataId').val()
        dataType : 'json', 
...
//		  	         ];
	...
	$(function() {
		...
		datagrid = $('#datagrid').treegrid({
			url :  getContextPath()+'/oaAsset/listOAATree.do',//list.do',getContextPath()+'/organization/listOSTree.do',
			title : '',
			iconCls : 'icon-save',
//			pagination : true,// 进度条
//			pageSize : 10,// 每页显示10条
...
//			initialState:"collapsed",
			...
			frozenColumns : [ [{
				...
				field:"text",title:'资产类别',width:300
			}]],
			columns:[[
//			{
...
//			},{
			   field:"assetUnit",title:'单位',width:180,sortable:true
			},{
	...
	function remove() {
		...
		if (node) {
			$.messager.confirm('询问', '您确定要删除【' + node.text + '】？', function(b) {
				if (b) {
					$.ajax({
						url : getContextPath()+'/oaAsset/delete.do',
						data : {
							id : node.id
						},
						cache : false,
