...
import com.slianOAsys.service.HttpJson;
...
public class CoreController {
	...
	public @ResponseBody String insert(HttpServletRequest request) throws UnsupportedEncodingException {
		...
		/*JSONObject jAuthInfo = JSONObject.fromObject( request.getParameter("inspectionrecord"));
		InspectionRecord inspectionrecord =  (InspectionRecord)JSONObject.toBean(jAuthInfo,InspectionRecord.class);
		 */
		...
		inspectionrecord.setTemperatureIn(request.getParameter("temperatureColdIn"));
		inspectionrecord.setTemperatureOut(request.getParameter("temperatureColdOut"));
	...
	public @ResponseBody Object downloadMedia(HttpServletRequest request){
		...
		String basePath=request.getSession().getServletContext().getRealPath("/").substring(0, request.getSession().getServletContext().getRealPath("/").lastIndexOf("bspheis-wechat"))+"/bspheis-uploads/images/";
	...
	public JSONObject Onload(HttpServletRequest request){
		...
		JSONArray Ja = CoreServiceImpl.onload(msg);
		msg="";
		...
		return CoreServiceImpl.onload(code);	
	}
